...
(function() {
  ...
  angular
    .module('merchantApp')
    .controller('ApplicationAreasListController', ApplicationAreasListController);

  ApplicationAreasListController.$inject = ['$compile', '$scope', '$stateParams', 'toaster', 'ApplicationService', 'DTOptionsBuilder', 'DTColumnBuilder', '$uibModal'];

  function ApplicationAreasListController($compile, $scope, $stateParams, toaster, ApplicationService, DTOptionsBuilder, DTColumnBuilder, $uibModal) {
    var vm = this;
    ...
    function initDt() {
      ...
      vm.dtColumns = [
        ...
        DTColumnBuilder.newColumn('orgRegion.data.name').withTitle('地域'),
    ...
    function dtAjax(data, callback, settings) {
      ...
      ApplicationService.orgRegionScores(params)
        .then(function(res) {
    ...
    function actionsHtml(data, type, full, meta) {
      ...
      return '<button class="btn btn-white btn-xs" uib-tooltip="编辑" ui-sref="admin.application.areasEdit({id: ' + data.id + '})">' +
